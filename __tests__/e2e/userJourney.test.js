...
jest.mock('../../src/core/services/prismaClient', () => ({
  ...
  specialOffer: { findMany: jest.fn(async () => []), findUnique: jest.fn(async () => null), findFirst: jest.fn(async () => null), update: jest.fn(), updateMany: jest.fn() },
...
jest.mock('../../src/core/services/cacheHelper', () => ({ getOrSet: jest.fn((key, fn) => fn()), invalidateTourCaches: jest.fn(() => Promise.resolve()), invalidateKeys: jest.fn(() => Promise.resolve()) }));
...
const { createPaymentIntent, calculateCommission, processStripeWebhook } = require('../../src/core/services/stripeHelpers');
const { enqueueEmail, enqueueNotification, enqueueEvent } = require('../../src/core/services/queue');
...
describe('E2E: Full User Journey', () => {
  ...
  it('completes the full journey: search → auth → cart → booking → webhook → email', async () => {
    ...
    prisma.cartItem.upsert.mockResolvedValue({
      ...
      travelDate: '2027-07-01', travelers: { adults: 2 },
    ...
    const cartRes = await request(app)
      ...
      .send({ tourId: 'tour-e2e-1', travelDate: '2027-07-01', travelers: { adults: 2 } });
    ...
    const bookingRes = await request(app)
      .post('/api/bookings')
      .set('Authorization', 'Bearer full-journey-token')
      .send({ tourId: 'tour-e2e-1', travelDate: '2027-07-01', travelers: { adults: 2 }, paymentMethodId: 'pm_full' });
