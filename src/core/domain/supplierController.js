...
const { sendSupplierStatusEmail } = require('../services/emailService');
...
const { isGhanaSupplier } = require('../services/supplierCountry');
const {
  parseDocuments,
  parseVehiclePhotos,
  parseVehicles,
...
exports.applyToBeSupplier = catchAsync(async (req, res, next) => {
  ...
  const { verificationSupplierApplication } = require('../services/channelEmbeds');
...
exports.reviewApplication = catchAsync(async (req, res, next) => {
  ...
  if (action === 'approve' && isGhanaSupplier(supplierProfile.businessInfo?.country)) {
    try {
      const { enqueueGhanaPublish } = require('../services/queue');
      const tours = await prisma.tour.findMany({
