  ...
  const [bookings, totalCount, counts] = await Promise.all([
    prisma.booking.findMany({
      ...
      select: {
        ...
        createdAt: true,
        updatedAt: true,
        leadTravelerName: true,
        leadTravelerEmail: true,
...
exports.chargePayLaterBooking = catchAsync(async (req, res, next) => {
  ...
  if (intent.status === 'succeeded') {
    ...
    const { handlePaymentSucceeded } = require('../services/stripeHelpers');
    try {
  ...
  if (confirmed.status === 'succeeded') {
    // Settle the booking
    const { handlePaymentSucceeded } = require('../services/stripeHelpers');
    const { enqueueNotification, enqueueEvent, enqueueEmail } = require('../services/queue');
    const { notifyAdmin } = require('../services/adminNotificationService');
    const { logActivity } = require('../services/auditLogger');
