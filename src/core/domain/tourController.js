...
const { createSlug, validateTourData, normalizeProductPayload, validateStoredPricing, rebuildSchedulePrices, reconcileAvailability, durationToMinutes, cheapestRetailPrice } = require('../services/tourHelpers');
const { cancelPaymentIntent } = require('../services/stripeHelpers');
...
const { variantsFor } = require('../services/attractionMatch');
const { destinationCityFromPatch } = require('../services/destinationCities');
const eventEmitter = require('../services/eventEmitter');

const { rankTourIdsBySearch } = require('../services/fullTextSearch');
const {
  computeDayEntry,
  parseBlob,
  travelerCount,
  toUtcDate,
  BOOKABLE_STATUSES,
} = require('../services/availabilityCore');
...
exports.getAllTours = catchAsync(async (req, res, next) => {
  ...
  if (req.query.search || req.query.category || req.query.location || (req.query.lat && req.query.lng)) {
    const { deriveSessionId } = require('../services/eventEmitter');
    enqueueEvent({
...
exports.createTour = catchAsync(async (req, res, next) => {
  ...
  const { validateTourOptions } = require('../services/tourOptions');
  ...
  setImmediate(async () => {
    try {
      ...
      const { enqueueAiScoring, enqueueGhanaPublish, enqueueTravioAfricaPublish } = require('../services/queue');
...
exports.submitTourForReview = catchAsync(async (req, res, next) => {
  ...
  const { enqueueGhanaPublish, enqueueTravioAfricaPublish } = require('../services/queue');
...
exports.validatePromoCode = catchAsync(async (req, res, next) => {
  ...
  const { findBestDiscount, findApplicableOffers } = require('../services/specialOfferEngine');
