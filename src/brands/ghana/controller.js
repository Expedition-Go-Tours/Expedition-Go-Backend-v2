...
const AppError = require('../../core/services/appError');
const cache = require('../../core/services/cacheHelper');
const { haversineKm, resolveCityCentroid } = require('../../core/services/locationGeo');
...
const getTours = catchAsync(async (req, res) => {
  ...
  const result = await cache.getOrSet(cacheKey, async () => {
    ...
    if (placeQuery) {
      const { resolved, ids, localIds, nearIds, regionFallback } = await placeTourIds(placeQuery, { ghanaOnly: true });
      if (resolved && ids) {
    ...
    if (mood) {
      const KEYWORD_CATEGORIES = require('../../core/services/keywordCategories');
    ...
    if (effectiveSearch) {
      ...
      const bm25 = require('../../core/services/bm25Index');
      if (bm25.isReady()) {
    ...
    let records = await prisma.travioGhanaTour.findMany({
    ...
    const totalCount = await prisma.travioGhanaTour.count({ where: { isActive: true, tour: tourWhere } });
...
const getTourBySlug = catchAsync(async (req, res, next) => {
  ...
  const result = await cache.getOrSet(DETAIL_CACHE_KEY(slug), async () => {
    ...
    photos: Array.isArray(t.photos) ? t.photos : [],
      ...
      options: require('../../core/services/tourOptions').optionSummaries(t),
...
const confirmBooking = catchAsync(async (req, res, next) => {
  ...
  const expTour = await prisma.travioGhanaTour.findUnique({
    where: { tourId },
    select: { isActive: true },
  });
  if (!expTour?.isActive) {
    return next(new AppError('Tour is not available on Travio Ghana', 400));
  }
  ...
  if (req.body.optionId) {
    const appliedG = require('../../core/services/tourOptions').applyOption(tour, String(req.body.optionId));
  ...
  require('../../core/services/payLaterLeadTime').assertPayLaterLeadTime({ paymentTiming, startAt });
  ...
  if (paymentTiming === 'later') {
    ...
    notifyAdmin({
      ...
      title: 'New Travio Ghana Booking (Reserve now, pay later)',
  ...
  prisma.checkoutDraft
    ...
    .catch((err) => console.error('[Travio Ghana] Failed to store Checkout Session id:', err.message));
...
const getMyBookings = catchAsync(async (req, res, next) => {
  ...
  const decorated = bookings.map((b) =>
    withChoiceToken({ ...b, refundState: bookingRefundState(b) }),
  );
...
const getBooking = catchAsync(async (req, res, next) => {
  ...
  const decorated = withChoiceToken({ ...booking, refundState: bookingRefundState(booking) });
  ...
  const result = await prisma.$transaction(async (tx) => {
    const updated = await tx.booking.update({
      ...
      data: {
        ...
        // supplier's cancellation rate is never hit by the customer's own act.
  ...
  if (needsRefund) {
    try {
      ...
      await prisma.booking.update({
        where: { id },
        data: { paymentStatus: 'REFUNDED', refundAmount, refundedAt: new Date(), refundStatus: 'SUCCEEDED' },
      });
    } catch (refundErr) {
      console.error(`[Travio Ghana] Stripe refund failed for booking ${id}:`, refundErr.message);
...
const updateBookingStatus = catchAsync(async (req, res, next) => {
  ...
  enqueueNotification({
    ...
    data: { bookingId: id, status, source: 'ghana' },
  });

  // Completed + paid → nudge the customer to write a review.
  if (status === 'COMPLETED') {
    const { enqueueReviewRequest } = require('../../core/services/reviewRequestNotify');
